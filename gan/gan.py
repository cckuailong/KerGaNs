...
class GAN:
    def __init__(self, img_shape, sample_shape=(5,5), latent=128, g_optimizer=Adam(0.0002, 0.5), d_optimizer=Adam(0.0002, 0.5), g_loss='binary_crossentropy', d_loss='binary_crossentropy'):
        if type(img_shape) == tuple and len(img_shape) == 3:
            ...
            print("[Error] Param 'img_shape' should be a trple set, eg. (28,28,1)")
            sys.exit(1)
        
        if type(sample_shape) == tuple and len(sample_shape) == 2:
            self.sample_shape = sample_shape
        else:
            print("[Error] Param 'sample_shape' should be a trple set, eg. (5,5)")
            sys.exit(1)
...
if __name__ == "__main__":
    ...
    gan.train(data=X_train, epochs=3000, batch_size=32, sample_interval=300)