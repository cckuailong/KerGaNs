...
class DISCOGAN:
    def __init__(self, img_shape, sample_shape=(2,3), g_optimizer=Adam(0.0002, 0.5), d_optimizer=Adam(0.0002, 0.5), g_loss=['mse', 'mse', 'mae', 'mae', 'mae', 'mae'], d_loss='mse'):
        ...
        # Get generator model
        self.g_AB = Generator(self.img_shape, self.gf).modelling()
        self.g_BA = Generator(self.img_shape, self.gf).modelling()
    ...
    def train_one_epoch(self, epoch, batch_size, valid, fake):
        for batch_i, (imgs_A, imgs_B) in enumerate(self.data_loader.load_batch(batch_size)):
            ...
            # Plot the progress
            print ("[Epoch %d] [Batch %d/%d] [D loss: %f, acc: %3d%%] [G loss: %05f, adv: %05f, recon: %05f, id: %05f]" \
                                                                        % ( epoch,
...
class Generator:
    ...
    def modelling(self):
        ...
        output_img = Conv2D(self.img_shape[2], kernel_size=4, strides=1, padding='same', activation='tanh')(u4)
...
class Discriminator:
    ...
    def modelling(self):
        ...
        d1 = self.d_layer(img, self.df, bn=False)
